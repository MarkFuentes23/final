...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo "<script>alert('Invalid email format.'); window.location.href='/includes/admin/profile/profile_setting.php';</script>";
    exit();
}
...
if (!empty($password)) {
    ...
    if (strlen($password) < 8) {
        echo "<script>alert('Password must be at least 8 characters long.'); window.location.href='/includes/admin/profile/profile_setting.php';</script>";
        exit();
    }
...
if ($stmt->execute()) {
    ...
    echo "<script>alert('Profile updated successfully.'); window.location.href='/includes/admin/profile/profile_setting.php';</script>";
} else {
    echo "<script>alert('Error updating profile. Please try again.'); window.location.href='/includes/admin/profile/profile_setting.php';</script>";
}
